...
В `docker-compose.yml` настраиваются координаты, название места, часовой
пояс, период опроса и `FRAME_LANG`. Состояния на диске нет — прогноз
хранится в памяти и заново запрашивается у Open-Meteo при каждом
старте контейнера, поэтому volume не нужен.
